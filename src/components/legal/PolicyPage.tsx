...
export function PolicyPage({
  ...
  return (
    <>
      <section className="relative isolate overflow-hidden bg-[#050b16] py-16 text-white md:py-20">
        ...
        <Container>
          <div className="grid items-end gap-10 lg:grid-cols-[1fr_auto]">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[.045] px-5 py-4 backdrop-blur-xl">
              <p className="text-[.62rem] font-extrabold uppercase tracking-[.15em] text-cyan-200">Last updated</p>
              <p className="mt-1 text-sm font-semibold text-white">{updated}</p>
            </div>
          </div>
        </Container>
      </section>
      ...
      <section className="light-grid bg-[#f5f8fd] py-14 md:py-20">
        <Container>
          <div className="grid gap-8 lg:grid-cols-[17rem_minmax(0,1fr)] lg:items-start">
            <aside className="rounded-3xl border border-slate-200 bg-white p-5 shadow-card lg:sticky lg:top-24">
            ...
            <div className="space-y-5">
              {sections.map((section, index) => (
                <article id={section.id} key={section.id} className="scroll-mt-28 rounded-3xl border border-slate-200 bg-white p-6 shadow-card sm:p-8">
                  <div className="flex items-start gap-4">
                    <span className="grid size-10 shrink-0 place-items-center rounded-xl border border-blue-100 bg-blue-50 text-xs font-black text-electric">{String(index + 1).padStart(2, '0')}</span>
